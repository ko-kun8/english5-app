    ...
    with CSV_FILE.open(encoding="utf-8-sig") as file:
        ...
        if type_key and english_key and japanese_key:
            for row in reader:
                ...
                item_type = normalize_column_name(row.get(type_key, "")).strip().lower()
...
def build_quiz_distractors(english: str, correct_japanese: str, count: int = 3) -> list[str]:
    ...
    for key, raw_entry in WORDS.items():
        ...
        if japanese and japanese != correct_japanese:
            others.append((key, japanese))
...
def setup_quiz_choices():
    ...
    if len(wrong_answers) < 3:
        ...
        for key, raw_entry in WORDS.items():
            ...
            entry = raw_entry if isinstance(raw_entry, dict) else {"type": "word", "japanese": raw_entry}
            japanese = entry.get("japanese", "")
            if japanese and japanese != correct and japanese not in wrong_answers:
                fallback.append(japanese)
    ...
    while len(wrong_answers) < 3:
        ...
        for key, raw_entry in WORDS.items():
            ...
            entry = raw_entry if isinstance(raw_entry, dict) else {"type": "word", "japanese": raw_entry}
            japanese = entry.get("japanese", "")
            if japanese and japanese not in wrong_answers and japanese != correct:
                fallback.append(japanese)
        random.shuffle(fallback)
        for meaning in fallback:
            if len(wrong_answers) >= 3:
