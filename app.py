...
def get_current_word() -> str | None:
    ...
    # 回答済みの場合は、現在の単語を維持する（pick_random_wordを避ける）
        ...
        if st.session_state.quiz_game_finished and st.session_state.quiz_result_summary:
            ...
            if st.session_state.quiz_answered:
                ...
                if st.session_state.quiz_was_correct:
                    ...
                    st.markdown('<div class="quiz-result-correct">⭕ せいかい！</div>', unsafe_allow_html=True)
                    
                    # 英単語の表示
                    st.markdown(f'<div style="text-align: center; font-size: 2rem; font-weight: 800; color: #4a90d9; margin: 1rem 0;">{current_english}</div>', unsafe_allow_html=True)
                    
                    # 日本語（漢字（ひらがな））の表示
                    st.markdown(f'<div style="text-align: center; font-size: 1.5rem; font-weight: 700; color: #2d8a2d; margin-bottom: 1rem;">{display_meaning}</div>', unsafe_allow_html=True)
                    ...
                    st.markdown('<div class="quiz-result-wrong">❌ ざんねん！</div>', unsafe_allow_html=True)
                    
                    # あなたの答え
                    # 答えも念のため整形（小学生モード設定に準拠）
                    ...
                    st.markdown(f'<div style="text-align: center; font-size: 1.2rem; font-weight: 700; color: #d64545; margin-bottom: 0.5rem;">{display_selected}</div>', unsafe_allow_html=True)
                    
                    st.markdown('<div style="text-align: center; color: #ccc;">────────────</div>', unsafe_allow_html=True)
                    
                    # 正解
                    st.markdown('<div style="text-align: center; color: #2d8a2d; font-weight: 800; margin-top: 0.5rem;">⭕ 正解</div>', unsafe_allow_html=True)
                    
                    st.markdown(f'<div style="text-align: center; font-size: 1.5rem; font-weight: 700; color: #2d8a2d; margin-bottom: 0.5rem;">{display_meaning}</div>', unsafe_allow_html=True)
                    
                    st.markdown('<div style="text-align: center; color: #ccc; margin-bottom: 0.5rem;">────────────</div>', unsafe_allow_html=True)
                    
                    # 英単語
                    st.markdown(f'<div style="text-align: center; font-size: 1.5rem; font-weight: 800; color: #4a90d9; margin-bottom: 1.5rem;">{current_english}</div>', unsafe_allow_html=True)
